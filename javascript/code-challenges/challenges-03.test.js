...
const typeNum = (arr) => {
  const numbersArray = arr.filter(item => typeof item === 'number');
  return numbersArray;
};
...
const containsAnd = (arr) => {
  const filteredArray = arr.filter(item => item.includes('and'));
...
const notInFirstArray = (forbiddenValues, arr) => {
  const filteredArray = arr.filter(item => !forbiddenValues.includes(item));
  return filteredArray;
};
