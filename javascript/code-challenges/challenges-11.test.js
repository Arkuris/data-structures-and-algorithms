...
const count = (target, input) => {
  return input.reduce((count, array) => {
    return count + array.reduce((subArrayCount, num) => {
      if (num === target) {
...
const totalSum = (input) => {
  return input.reduce((total, currentArray) => {
    const sumOfArray = currentArray.reduce((acc, currentValue) => {
...
let findShortest = (data) => {
  const filteredData = data.filter((character) => {
    return !isNaN(parseInt(character.height));
  });
  const characterHeights = filteredData.map((character) => {
    return {
      name: character.name,
      height: parseInt(character.height),
    };
  });
  const shortestCharacter = characterHeights.reduce((shortest, current) => {
